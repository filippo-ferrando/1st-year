 ...
 */
public class TorreHanoiConfigurazioniNonOttimale {
    ...
    public static String muoviTorre(int n, int s, int aus, int d,
            String mosse) {

        if (n == 1)
            mosse = sposta(s, d, mosse);
        else {
            mosse = muoviTorre(n - 1, s, aus, d, mosse);
            mosse = sposta(s, aus, mosse);
            mosse = muoviTorre(n - 1, d, aus, s, mosse);
            mosse = sposta(aus, d, mosse);
            mosse = muoviTorre(n - 1, s, aus, d, mosse);
        }
        return mosse;
    }
	...
	public static void configurazioni(String mosse, int numeroDischi) {
		...
		// scrive le lettere nelle celle che rappresentano il primo piolo
		for (int r = 0; r < configurazione.length; r++)
			configurazione[r][0] = Character.toChars(configurazione.length - 1 - r + 65)[0];			
		// scrive le configurazioni vuote degli altri pioli
		for (int r = 0; r < configurazione.length; r++)
			for (int c = 1; c < NUMERO_PIOLI; c++)
				configurazione[r][c] = ' ';

        // Stampa una configurazione dopo l'altra, assumendo che esse siano
        // state accodate una dopo l'altra in mosse dal metodo tH
...
//			System.out.println(toStringLaTeX(configurazione, mossa));
			...
			if (trovataCima) // la destinazione ha almeno un disco
				cimaDest++;
...
//		System.out.println(toStringLaTeX(configurazione, ""));
	...
	public static void main(String[] args) {
		...
		for (int numeroDischi = 4; numeroDischi <= numeroMaxDischi; numeroDischi++) {
			...
			String configurazioni = muoviTorre(numeroDischi, PioloA, PioloB, PioloC, "");
//			System.out.println(config);
			configurazioni(configurazioni, numeroDischi);
			System.out.println("\n Numero totale mosse: " + numeroMosse);
		}
	}
}
