 ...
 * dicotomica, ad esempio, risolve i sottoproblemi, indipendentemente sulle due
 * parti, e li ricompone.
 */
public class CopyOfEserciziDicotomiciDerivatiCorrettezza {
	...
	private static int minimo(int[] a, int s, int d) {
		if (s == d)
			...
			int mS = minimo(a, s, (s + d) / 2); // minimo nella porzione sinistra
			int mD = minimo(a, (s + d) / 2 + 1, d); // minimo nella porzione destra
